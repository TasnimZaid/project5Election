...
const calculateSeats = async (totalAllocatedSeats) => {
    ...
    const totalPartyVotes = partyVotes.reduce((sum, item) => sum + item.voted_count, 0);
  
    // Threshold is 2.5% of the total votes
    const threshold = 0.025 * totalPartyVotes;
  
    // 2. Filter parties that pass the threshold
    const listsPassingThreshold = partyVotes.filter(item => item.voted_count >= threshold);
  
    // 3. Calculate the total votes of passing lists
    const totalVotesOfPassingLists = listsPassingThreshold.reduce((sum, row) => sum + row.voted_count, 0);
  
    // 4. Assign seats based on the formula
    const results = [];
    for (let list of listsPassingThreshold) {
      let seats = (totalAllocatedSeats / totalVotesOfPassingLists) * list.voted_count;
